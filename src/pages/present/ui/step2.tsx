...
import { type IMemories, useFriendMemories } from "@features/memories/api/useFriendMemories";
...
export const PresentStep2 = ({ id = "" }: PresentStep2Props) => {
  ...
  const renderItem = (item: IMemories) => {
    ...
    return item.imageUrl ? (
      <PhotoBox key={item.id} text={item.content} date={item.createdAt} photo={item.imageUrl} select={memories.includes(itemId)} onClick={() => toggle(itemId)} />
    ) : (
      <TextBox key={item.id} text={item.content} date={item.createdAt} select={memories.includes(itemId)} onClick={() => toggle(itemId)} />
    );
  };
