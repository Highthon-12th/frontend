  ...
  return (
    <div className="flex flex-col px-5 pt-5 gap-5 min-h-full">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-grey font-medium">기록은 최대 6개까지 선택할 수 있어요</p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <SelectButton
          ...
          onClick={() => { setSelected("kakao"); setGiftOption("kakao"); }}
        />
        <SelectButton
          icon={<img src={phone} alt="직접 고르기" className="w-10 h-10" />}
          label="따로 직접 고를게요"
          select={selected === "direct"}
          onClick={() => { setSelected("direct"); setGiftOption("direct"); }}
        />
      </div>

      <p className="mt-auto text-center text-sm text-grey cursor-pointer" onClick={handleReset}>선물을 다시 고르고 싶어요</p>
    </div>
  );
};
